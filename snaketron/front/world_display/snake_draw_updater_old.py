...
class SnakeDrawUpdater(EventDispatcher):
    ...
    animated_wrapping_head_x = NumericProperty(0.)
    animated_wrapping_head_y = NumericProperty(0.)
    animated_wrapping_head_pos = ReferenceListProperty(animated_wrapping_head_x, animated_wrapping_head_y)
    animated_wrapping_head_rgb = ListProperty([0., 0., 0.])

    animated_wrapping_tail_x = NumericProperty(0.)
    animated_wrapping_tail_y = NumericProperty(0.)
    animated_wrapping_tail_pos = ReferenceListProperty(animated_wrapping_tail_x, animated_wrapping_tail_y)
    animated_wrapping_tail_rgb = ListProperty([0., 0., 0.])
    ...
    def __init__(
        ...
        self.animated_wrapping_head: Rectangle = None
        self.animated_wrapping_tail: Rectangle = None
        self.tail_animation: Optional[Animation] = None
        self.head_animation: Optional[Animation] = None
        self.wrapping_head_animation: Optional[Animation] = None
        self.wrapping_tail_animation: Optional[Animation] = None

        # color system
        self.colors = colors
        self.animated_head_color = Color(*self.colors.head)
        self.animated_tail_color = Color(*self.colors.tail)
        self.body_color = Color(*self.colors.tail)
    ...
    def on_body_rgb(self, _, value):
        self.body_color.rgb = value
    ...
    def _stop_animations(self) -> None:
        ...
        if self.wrapping_head_animation is not None:
            self.wrapping_head_animation.stop(self)
        if self.wrapping_tail_animation is not None:
            self.wrapping_tail_animation.stop(self)
    ...
    def _init_color(self) -> None:
        ...
        self.body_rgb = self.colors.tail
    ...
    def _init_body(self) -> None:
        ...
        self.body_color = Color(*self.body_rgb)
        self.instr_back.add(self.body_color)
        for pos in cells:
    ...
    def _init_animated_tail(self) -> None:
        ...
        self.animated_wrapping_tail = self._square(end_tail_pos)
        self.animated_wrapping_tail_pos = self.animated_wrapping_tail.pos
        self.animated_wrapping_tail_color = Color(*self.animated_wrapping_tail_rgb)

        self.instr_fore.add(self.animated_tail_color)
        self.instr_fore.add(self.animated_tail)
        self.instr_fore.add(self.animated_wrapping_tail_color)
        self.instr_fore.add(self.animated_wrapping_tail)
    ...
    def _animate_head_wrap(self, wrap_dir: Direction, time_step: float) -> None:
        wrap_out_start_pos = (self.head_pos[0]-wrap_dir[0], self.head_pos[1]-wrap_dir[1])
        self.animated_head_pos = self.display.pos_to_coord(wrap_out_start_pos)
        self._animate_head(time_step)

    def _animate_tail_wrap(self, wrap_dir: Direction, time_step: float) -> None:
        ...
    ...
    def _animate_decay(self, time_step: float) -> None:
        ...
        self.decay_animation = Animation(
            ...
            body_rgb=self.colors.tail_decay_final, duration=d, t=animation_transition
        )
        ...
        self.body_rgb = self.colors.tail_decay_first
    ...
    def update_draw_snake_wrap(self, event: SnakeMovement, time_step: float) -> None:
        # self.update_draw_snake_move(event, time_step)
        self._update_body(event.new_head_pos, event.growth)
        self._animate_head_wrap(event.new_dir, time_step)
        self._animate_tail_wrap(event.new_dir, time_step)
